...
void main() {
  ...
  group('RepeaterFormatHelper.formatCtcss', () {
    ...
    test('common CTCSS tone 67.0 Hz', () {
      expect(RepeaterFormatHelper.formatCtcss(67.0), '67.0 Hz');
    });
  ...
  group('RepeaterFormatHelper.formatShareMessage', () {
    Repeater makeRepeater({
      String? callsign,
      String? name,
      int frequencyHz = 145625000,
      String? locality,
      String? region,
    }) {
      return Repeater(
    ...
    test('uses callsign when available', () {
      final repeater = makeRepeater(callsign: 'IR5UBY', name: 'Monte Serra');
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('IR5UBY'));
      expect(result, isNot(contains('Monte Serra')));
    });

    test('falls back to name when callsign is null', () {
      final repeater = makeRepeater(name: 'Monte Serra');
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('Monte Serra'));
    });

    test('falls back to "Repeater" when both callsign and name are null', () {
      final repeater = makeRepeater();
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('Repeater'));
    });

    test('includes formatted frequency', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
      ...
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('145.625 MHz'));
    });

    test('includes location with locality and region', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        locality: 'Firenze',
        region: 'Toscana',
      ...
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('Firenze, Toscana'));
    });

    test('includes location with only locality', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        locality: 'Firenze',
      );
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('Firenze'));
    });

    test('includes location with only region', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        region: 'Toscana',
      );
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('Toscana'));
    });

    test('omits location when both are null', () {
      final repeater = makeRepeater(callsign: 'IR5UBY');
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      // Should not have an extra line between frequency and blank line
      final lines = result.split('\n');
      expect(lines[0], 'IR5UBY');
      expect(lines[1], '145.625 MHz');
      expect(lines[2], ''); // blank line before app message
      expect(lines[3], 'Download HamQRG');
    });

    test('includes app message at the end', () {
      final repeater = makeRepeater(callsign: 'IR5UBY');
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, endsWith('Download HamQRG'));
    });

    test('full message with all fields', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        frequencyHz: 430412500,
        locality: 'Firenze',
        region: 'Toscana',
      );
      final result =
    ...
    test('ignores empty string locality', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        locality: '',
        region: 'Toscana',
      ...
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('Toscana'));
      expect(result, isNot(contains(', Toscana')));
    });

    test('ignores empty string region', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        locality: 'Firenze',
        region: '',
      ...
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('Firenze'));
      expect(result, isNot(contains('Firenze, ')));
    });

    test('omits location when both are empty strings', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        locality: '',
        region: '',
    ...
    test('full message without location has correct line count', () {
      final repeater = makeRepeater(callsign: 'IR5UBY');
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result.split('\n').length, 4);
    ...
    test('full message with location has correct line count', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        locality: 'Firenze',
        region: 'Toscana',
    ...
    test('name with spaces is preserved', () {
      final repeater = makeRepeater(name: 'Monte Serra');
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      final lines = result.split('\n');
      expect(lines[0], 'Monte Serra');
    });

    test('UHF frequency in share message', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        frequencyHz: 430412500,
      );
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, contains('430.4125 MHz'));
    });

    test('multiline app message', () {
      final repeater = makeRepeater(callsign: 'IR5UBY');
      final result = RepeaterFormatHelper.formatShareMessage(
        repeater,
        'Download HamQRG\nhttps://example.com',
      );
      expect(result, endsWith('Download HamQRG\nhttps://example.com'));
    ...
    test('does not end with trailing newline', () {
      final repeater = makeRepeater(
        callsign: 'IR5UBY',
        locality: 'Firenze',
        region: 'Toscana',
    ...
    test('message starts with first line', () {
      final repeater = makeRepeater(callsign: 'IR5UBY');
      final result =
          RepeaterFormatHelper.formatShareMessage(repeater, 'Download HamQRG');
      expect(result, startsWith('IR5UBY\n'));
    });
  });
}
